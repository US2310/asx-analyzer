...
const ASX_STOCKS = [
  ...
  { code: "ANZ",  name: "ANZ Banking Group",           sector: "Financials" },
  ...
  { code: "WOW",  name: "Woolworths Group",            sector: "Consumer" },
  { code: "FMG",  name: "Fortescue Metals",            sector: "Materials" },
  { code: "GMG",  name: "Goodman Group",               sector: "Property" },
  { code: "TCL",  name: "Transurban",                  sector: "Infrastructure" },
  { code: "STO",  name: "Santos",                      sector: "Energy" },
  { code: "XRO",  name: "Xero",                        sector: "Technology" },
  { code: "ALL",  name: "Aristocrat Leisure",          sector: "Consumer" },
  { code: "COL",  name: "Coles Group",                 sector: "Consumer" },
  { code: "REA",  name: "REA Group",                   sector: "Technology" },
  ...
  { code: "QBE",  name: "QBE Insurance",               sector: "Financials" },
  { code: "IAG",  name: "Insurance Australia Group",   sector: "Financials" },
  { code: "CPU",  name: "Computershare",               sector: "Financials" },
  { code: "SEK",  name: "SEEK",                        sector: "Technology" },
];
...
function generateSignals(data, indicators) {
  const { sma20, sma50, sma200, ema9, ema21, rsi, macd, bb, stoch } = indicators;
  const signals = [];
  ...
  // SMA 20/50 cross
  if (sma20[last] && sma50[last] && sma20[prev] && sma50[prev]) {
    if (sma20[prev] < sma50[prev] && sma20[last] > sma50[last]) {
      signals.push({ type: "BUY", indicator: "Golden Cross", detail: "SMA20 crossed above SMA50 — major bullish trend signal", strength: "Strong" });
      bullish += 3;
    } else if (sma20[prev] > sma50[prev] && sma20[last] < sma50[last]) {
      signals.push({ type: "SELL", indicator: "Death Cross", detail: "SMA20 crossed below SMA50 — major bearish trend signal", strength: "Strong" });
      bearish += 3;
    } else { sma20[last] > sma50[last] ? bullish++ : bearish++; }
  }

  // Price vs SMA200 (long-term trend)
  if (sma200[last]) {
    const close = data[last].close;
    if (close > sma200[last] * 1.02) {
      signals.push({ type: "BUY", indicator: "SMA200 Trend", detail: `Price ${((close/sma200[last]-1)*100).toFixed(1)}% above 200-day MA — long-term uptrend confirmed`, strength: "Moderate" });
      bullish += 2;
    } else if (close < sma200[last] * 0.98) {
      signals.push({ type: "SELL", indicator: "SMA200 Trend", detail: `Price ${((1-close/sma200[last])*100).toFixed(1)}% below 200-day MA — long-term downtrend`, strength: "Moderate" });
      bearish += 2;
    }
  }
...
// ─── CANDLESTICK ──────────────────────────────────────────────────────────────
const CandleChart = ({ data, sma20, sma50, sma200, bbUpper, bbLower, vwap, theme: th }) => {
  ...
  const allVals = [
    ...data.map(d => d.high), ...data.map(d => d.low),
    ...sma20.filter(Boolean), ...sma50.filter(Boolean),
    ...bbUpper.filter(Boolean), ...bbLower.filter(Boolean),
  ];
  ...
  const linePath = (values) => {
    let d = '', inSeg = false;
    values.forEach((v, i) => {
      if (v != null) { d += (inSeg ? ' L' : 'M') + `${toX(i)},${toY(v)}`; inSeg = true; }
      else { inSeg = false; }
    });
    return d;
  };

  if (!dims.w || plotH <= 0) return <div ref={containerRef} style={{ width: '100%', height: 300 }} />;
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: 300 }}>
      <svg width={dims.w} height={dims.h}>
        ...
        {ticks.map((tick, i) => (
          <text key={i} x={PAD.left - 4} y={toY(tick) + 3.5} textAnchor="end" fontSize={9} fill={th.textDim}>{fmtPrice(tick)}</text>
        ))}
        {xLabels.map(({ i, label }) => (
          <text key={i} x={toX(i)} y={PAD.top + plotH + 16} textAnchor="middle" fontSize={9} fill={th.textDim}>{label}</text>
        ))}
        ...
        <path d={linePath(sma20)}   fill="none" stroke="#3b82f6" strokeWidth={1.5} strokeDasharray="5 3" />
        <path d={linePath(sma50)}   fill="none" stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="5 3" />
        <path d={linePath(sma200)}  fill="none" stroke="#f97316" strokeWidth={1.5} strokeDasharray="5 3" />
        ...
        <rect x={PAD.left} y={PAD.top} width={plotW} height={plotH} fill="none" stroke={th.border} strokeWidth={1} />
      </svg>
    </div>
  );
};
...
export default function ASXAnalyzer() {
  ...
  const loadStock = useCallback(async (stock, per = period) => {
    ...
    try {
      ...
      const sma50  = calcSMA(raw, 50);
      const sma200 = calcSMA(raw, 200);
      ...
      const inds   = { sma20, sma50, sma200, ema9, ema21, rsi, macd, bb, atr, stoch, vwap };
      setData(raw);
      setIndicators(inds);
  ...
  // Chart data merged
const display = data.slice(-90);
const visibleHighs = display.map(d => d.high);
const visibleLows  = display.map(d => d.low);
const rawHigh = display.length ? Math.max(...visibleHighs) : 0;
const rawLow  = display.length ? Math.min(...visibleLows)  : 0;
const pad     = (rawHigh - rawLow) * 0.05;
const chartHigh = rawHigh + pad;
const chartLow  = Math.max(0, rawLow - pad);

  const chartData = display.map((d, i) => {
    const gi = data.length - display.length + i;
    return {
      ...d,
      sma20:    indicators?.sma20[gi],
      sma50:    indicators?.sma50[gi],
      sma200:   indicators?.sma200[gi],
                    ...
                    <div style={{ ...S.card, display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 180 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: t.textDim, marginBottom: 5 }}>
                          <span>Bear {analysis.bearish}</span>
                          <span style={{ color: analysis.recColor }}>Bull Score {analysis.score}%</span>
                          <span>{analysis.bullish} Bull</span>
                        </div>
                    ...
                    {activeTab === "price" && (
                      <>
                        <div style={{ fontSize: 10, color: t.textDim, padding: "0 12px 10px", display: "flex", gap: 14, flexWrap: "wrap" }}>
                          ...
                          <span style={{ color: "#3b82f6" }}>- SMA20</span>
                          <span style={{ color: t.amber }}>- SMA50</span>
                        ...
                        {chartMode === "candle" && (
                          <ResponsiveContainer width="100%" height={300}>
                            <ComposedChart data={chartData} margin={{ left: 0, right: 8 }}>
                              ...
                              <YAxis tick={{ fontSize: 9, fill: t.textDim }} domain={([min, max]) => {
  const pad = (max - min) * 0.05;
  return [Math.max(0, min - pad), max + pad];
}} width={56} tickFormatter={v => {
  if (v >= 1000) return `$${(v/1000).toFixed(1)}k`;
  if (v >= 100)  return `$${v.toFixed(0)}`;
  if (v >= 10)   return `$${v.toFixed(2)}`;
  ...
  if (!d) return null;
  return <CandlestickBar {...props} open={d.open} close={d.close} high={d.high} low={d.low} chartHigh={chartHigh} chartLow={chartLow} chartHeight={props.background?.height || 260} theme={t} />;
}} />;
                              }} />
                              <Line dataKey="sma20"  stroke="#3b82f6"  dot={false} strokeWidth={1.5} strokeDasharray="5 3" name="SMA20" />
                              <Line dataKey="sma50"  stroke={t.amber}  dot={false} strokeWidth={1.5} strokeDasharray="5 3" name="SMA50" />
                              <Line dataKey="sma200" stroke="#f97316"  dot={false} strokeWidth={1.5} strokeDasharray="5 3" name="SMA200" />
                        ...
                        {chartMode === "line" && (
                          <ResponsiveContainer width="100%" height={300}>
                            <ComposedChart data={chartData} margin={{ left: 0, right: 8 }}>
                              <CartesianGrid strokeDasharray="3 3" stroke={t.border} />
                              <XAxis dataKey="date" tick={{ fontSize: 9, fill: t.textDim }} tickFormatter={d => d.slice(5)} interval={Math.floor(chartData.length / 6)} />
                              <YAxis tick={{ fontSize: 9, fill: t.textDim }} domain={[chartLow, chartHigh]};
}} width={56} tickFormatter={v => {
  if (v >= 1000) return `$${(v/1000).toFixed(1)}k`;
  if (v >= 100)  return `$${v.toFixed(0)}`;
  if (v >= 10)   return `$${v.toFixed(2)}`;
  ...
  return `$${v.toFixed(4)}`;
                      ...
                      <div style={{ padding: "4px 12px 8px" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10, marginBottom: 16 }}>
                          {[
                            ...
                            { label: "EPS",          value: stockInfo.eps ? `A$${stockInfo.eps.toFixed(3)}` : "—" },
                            { label: "Dividend Yield",value: stockInfo.dividend ? `${(stockInfo.dividend * 100).toFixed(2)}%` : "—" },
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 8 }}>
                    {[
                      ...
                      { label: "SMA 20",     value: indicators?.sma20[data.length-1] ? `A$${indicators.sma20[data.length-1].toFixed(3)}` : "—" },
                      { label: "SMA 50",     value: indicators?.sma50[data.length-1] ? `A$${indicators.sma50[data.length-1].toFixed(3)}` : "—" },
